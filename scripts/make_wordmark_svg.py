"""
Render a short wordmark as an EXTRUDED 3D wordmark rasterized to ASCII, and
emit it as an SVG that animates on GitHub (SMIL only -- GitHub runs SVG
animations in <img>, but never JS).

Adapted from the same pipeline used by github.com/AVIVASHISHTA29's profile
(see docs/3d-ascii-wordmark.md for the full write-up of how this works):
draw the word with a bold TTF -> threshold to a mask -> extrude the mask
along +z into a surface voxel shell (front cap, back cap, boundary sides)
-> rotate / project each frame -> z-buffer splat into a character grid,
char picked by Lambert shading of the surface normal.
...
All modes open with the same left-to-right wipe, then differ in the rotation:
  rock   -- oscillates +/-11 deg around the rest pose, forever (this is the
            one wired into the README)
...
# blank rows above and below the art, padding the panel out to line up with
# the ASCII portrait beside it (see the sizing note at the bottom of the file).
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", 5))
CELL_W = 9.0
CELL_H = 15.5
# Outfit Bold: a free geometric sans (SIL OFL) bundled in assets/ so this
# renders identically on any machine/CI runner without relying on a system
# font -- the original used Futura, a macOS-only font, which isn't portable.
FONT_PATH = os.environ.get("WORDMARK_FONT", os.path.join(HERE, "..", "assets", "Outfit-Bold.ttf"))
...
TEXT = os.environ.get("WORDMARK_TEXT", "RMU")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">mehmet@github: ~$ ./wordmark.sh --3d</text>')
